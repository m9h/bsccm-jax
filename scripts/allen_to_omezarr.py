...
Z_SLICES = [28, 39, 50]                    # in-focus central slices -> 2D samples
...
def write_zarr(files, out_path):
    ...
    for fi, f in enumerate(files):
        ...
        for zi, z in enumerate(Z_SLICES):
            if z >= Z:
                continue
            bright = arr[C_BRIGHT, z].astype(np.float32)
            lamin = arr[C_LAMIN, z].astype(np.float32)
            for ti, (y0, x0) in enumerate(_tiles(bright)):
                ...
                data = np.stack([bt, lt])[None, :, None]      # (T=1, C=2, Z=1, Y, X)
                pos.create_image("0", data)
                n += 1
    plate.close()
    print(f"wrote {out_path}: {n} samples from {len(files)} files")
    return n
...
def main():
    ...
    write_zarr(train, args.out_train)
    write_zarr(val, args.out_val)
